...
def main():
    ...
    # 创建AI对象
    # ai = ChessAI.ChessAI(game.computer_team)
    # myai = MyAI.ChessAI(game.user_team)
    #创建用户AI对象
    if choice == "red":
        ai = ChessAI.ChessAI(game.computer_team) #对方设定为黑方
        myai = MyAI.ChessAI(game.user_team) #己方设定为红方
    elif choice == "black":
        ai = ChessAI.ChessAI(game.user_team) #己方设定为黑方
        myai = MyAI.ChessAI(game.computer_team) #对方设定为红方
    else :
        print("error input")
        return 0
    # 主循环
    while True:
        ...
        if game.show_win or game.show_draw : continue
