...
Page* BufferPoolManager::fetch_page(PageId page_id) {
    ...
    if (it != page_table_.end()) {
        frame_id_t frame_id = it->second;  // 获取页面所在的帧号
        Page* page = &pages_[frame_id];    // 从 pages_ 数组中定位页面
    ...
    // 2.     UpdatePage函数：若获得的可用frame存储的为dirty page，则须调用updata_page将page写回到磁盘
    update_page(&pages_[avaliable_frame_id], page_id, avaliable_frame_id);
    // 3.     调用disk_manager_的read_page读取目标页到frame
    Page* page = &(pages_[avaliable_frame_id]);  // 通过id获取page
...
bool BufferPoolManager::unpin_page(PageId page_id, bool is_dirty) {
    ...
    Page& page = pages_[it->second];  ////////////////////注意是引用
...
bool BufferPoolManager::delete_page(PageId page_id) {
    ...
    // 3.   将目标页数据写回磁盘，从页表中删除目标页，重置其元数据(data, is_dirty,
    // page_id)，将其加入free_list_，返回true 3.1  将目标页数据写回磁盘
    disk_manager_->write_page(page_id.fd, page_id.page_no, target_page.get_data(), PAGE_SIZE);
    // 3.2 从页表中删除目标页
    page_table_.erase(page_id);
    // 3.3 重置其元数据(data, is_dirty, page_id)
